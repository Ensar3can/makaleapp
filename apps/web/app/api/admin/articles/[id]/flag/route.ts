import { flagArticleBodySchema } from '@aip/validation';
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const body = flagArticleBodySchema.parse(await readJson(request));
    const result = await getModerationServices().flagArticle.execute({
